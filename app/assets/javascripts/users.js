$(function(){
  function addUser(user) {
    let html = `
      <div class="chat-group-form__search clearfix">
        <p class="chat-group-user__input">${user.name}</p>
  ...
  function addNoUser() {
    let html = `
      <div class="chat-group-form__search clearfix">
        <p class="chat-group-user__input">ユーザーが見つかりません</p>
      </div>
    `;
    $("#user-search-result").append(html);
  }
  function addDeleteUser(name, id) {
    let html = `
    <div class="chat-group-form__search clearfix" id="${id}">
      <p class="chat-group-user__input">${name}</p>
